...
void Game::DrawGame() //Borrar y ver que todavia se dibuja usando SFMLRender por el SetDebugDraw de Box2D implementado en el método InitPhysics
{
    // Dibujar el suelo
...
void Game::DoEvents()
{
    ...
    // Mantengo la pelota "despierta" constantemente
...
void Game::InitPhysics()
{
    ...
    //Obstaculos
    for (int i=0; i < 5; i++) { obstacle[i] = Box2DHelper::CreateTriangularStaticBody(phyWorld, b2Vec2(0.0f, 0.0f), 10.0f); }
    
    obstacle[0]->SetTransform(b2Vec2(30.0f, 15.0f), 25.0f);
    obstacle[1]->SetTransform(b2Vec2(80.0f, 70.0f), 90.0f);
    obstacle[2]->SetTransform(b2Vec2(20.0f, 80.0f), 0.0f);
    obstacle[3]->SetTransform(b2Vec2(70.0f, 30.0f), 35.0f);
    obstacle[4]->SetTransform(b2Vec2(50.0f, 50.0f), 50.0f);


    // Crear pelota

    ball = new Pelota(phyWorld, wnd);
